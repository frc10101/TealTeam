...
func insertMatchTeam(db *gorm.DB, eventID, teamID int, allianceColor string, position int) error {
	...
	shootingSpeed := []string{"fast", "medium", "slow"}[rand.Intn(3)]
	capacity := []string{"high", "medium", "low"}[rand.Intn(3)]
	hangLevel := []string{"none", "l1", "l2", "l3"}[rand.Intn(4)]
	autoHang := []string{"yes", "no"}[rand.Intn(2)]
	hangPosition := []string{"left", "center", "right"}[rand.Intn(3)]
