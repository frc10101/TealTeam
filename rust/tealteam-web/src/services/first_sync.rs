...
//!
//! A port of `internal/frc/sync.go` and `Services/FirstSyncService.cs`.
//!
//! Three entry points, for three moments:
//!
//! - [`sync_on_boot`] runs once at startup unless `FIRST_SYNC_ON_BOOT=false`;
//! - [`sync_now`] is the manual sync behind `POST /api/frc/sync`;
//! - [`sync_team_for_user`] runs on sign-in, sign-up and team lookup, and
//!   fetches only that team's events.
//!
//! What gets pulled is narrowed by `FIRST_EVENT_CODE`, `FIRST_TEAM_NUMBER` and
//! `FIRST_COUNTRY` (default USA), because a whole season of worldwide events
//! is far more than one team's Pi needs.
//!
//! Everything upserts, so syncing repeatedly is safe and is how the local
//! database stays usable while the venue's uplink comes and goes. A completed
//! team sync also kicks off a TBA stats sync for that team's events.
...
use super::first_api::{FirstApiClient, FirstEvent, FirstTeam};
use super::tba::TbaClient;
use super::tba_stats_sync;

const DEFAULT_COUNTRY: &str = "USA";

/// Sync did not run because FIRST credentials are not configured. Callers
/// treat this as a normal condition, not a failure.
#[derive(Debug)]
...
/// What a sync pulled in, for logs and the manual-sync response.
...
/// Startup sync. Bounded by a timeout and never fatal: a failure is logged
/// and the server starts anyway on whatever data it already has.
...
/// Full sync for the configured season and filters: events, then each
/// event's team roster, then the event/team links.
...
/// Syncs one team's events and rosters, then launches a background TBA stats
/// sync for those events.
///
/// Called on sign-in, sign-up and team lookup, so a user's own competition
/// data is present the first time they need it.
...
/// Fetches TBA stats and matches for the given events, if a `TBA_AUTH_KEY` is
/// configured.
...
/// Upserts events and returns their local ids keyed by event code.
...
/// Upserts one event, matched on its TBA key.
...
/// Upserts one team, matched on its team number, and returns its local id.
///
/// Also called directly by the submission form when a scout picks an event
/// whose roster has not been synced yet.
...
/// Links a team to an event, ignoring duplicates.
...
/// Parses a FIRST date, which may or may not carry a time component.
...
/// Environment variable with surrounding whitespace removed, empty if unset.
