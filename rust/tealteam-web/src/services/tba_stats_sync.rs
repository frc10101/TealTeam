//! Shared TBA statistics and match sync, a port of `Services/TbaStatsSync.cs`.
//!
//! Used by both the background loop ([`super::stats_syncer`]) and the per-team
//! sync that runs on sign-in ([`super::first_sync::sync_team_for_user`]).
//!
//! [`sync_team_stats_for_event`] writes `team_event_stats` — rank, record,
//! OPR/DPR/CCWM and the phase components. [`sync_event_matches`] writes
//! `matches`, which is both the assignment grid's schedule and the offline
//! fallback for the home-page schedule panel.
//!
//! Both upsert, so a partially-completed sync simply resumes on the next pass.
...
/// Pulls rankings and OPR data for one event and upserts a
/// `team_event_stats` row per team on its roster.
pub async fn sync_team_stats_for_event(
    ...
    let rankings_by_team: std::collections::HashMap<&str, &super::tba::RankingInfo> =
...
/// Pulls every match at an event and upserts it, including robot line-ups,
/// scores and scheduled times.
...
/// Ensures an event key carries its season prefix (`mndu` -> `2026mndu`),
/// since local rows may store either form.
...
/// Flattens TBA's (set, match) elimination numbering into the single match
/// number the schema stores.
...
/// TBA timestamps are Unix seconds, with 0 meaning "unknown".
...
/// Extracts the team number from the nth `frc####` key of an alliance.
